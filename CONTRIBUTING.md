...
Before we start, a heartfelt thank you ❤️ for taking the time out and considering contributing to [Elementa](https://elementa.dev). We welcome your contributions to our icon library! Now, please follow these guidelines to ensure a smooth process. We use pull-requests to accept any kind of contribution to Elementa.
...
4. In your PR, please only include the TSX code and not the SVG version. The Google Drive folder contains all the SVGs, and equivalent PNGs on a Google Cloud Storage bucket. Once your PR is merged, maintainers will take the SVG, convert it into PNG and upload it on the bucket.

5. Bump up the version of the library in `package.json`. Follow semantic versioning.

6. Create a PR with your changes. A side but important note here; if you are adding multiple icons, please group the PRs by category. Don't create a single PR having multiple icons across the categories. These PRs will unfortunately be closed my maintainers.
...
1. In the **elementa.dev** repo, open the category file:
  - `lib/utils/icons/[category].ts`
  (path is relative to the root of elementa.dev)
2. Add your icon entry in the existing array under `icons` field and also import the new icon from elementa-icons library.
  - Or add to a default export/array if that’s how icons are aggregated in your repo.
3. Save changes.


### 3.5.5 Run the dev server
...
### 3.5.7 Troubleshooting tips
- If using yarn link and you see stale code, unlink and relink or restart the dev server:
  - `yarn unlink --no-save elementa-icons` (in elementa.dev)
  - `yarn link elementa-icons` (in elementa.dev)
...
### 3.5.8 Verification checklist
- [ ] **elementa-icons** is linked or referenced locally by **elementa.dev**
- [ ] lib/utils/icons/[category].ts contains the new icon export
...
Elementa is hosted on Google Cloud Run powered by Firebase Hosting. The codebase is @ [elementa.dev](https://github.com/deriva-inc/elementa.dev). If you want to contribute to the website, please refer [elementa.dev | CONTRIBUTING](https://github.com/deriva-inc/elementa.dev/blob/main/CONTRIBUTING.md).
